...
def pytest_addoption(parser):
    parser.addoption(
        ...
        action="store_true",
        help=(
            "enable running doctests with additional "
    ...
    parser.addini(
        "doctest_rst", "Run the doctests in the rst documentation", default=False
    )
...
def pytest_configure(config):
    ...
    OutputChecker.rtol = max(
        float(config.getini("doctest_plus_rtol")),
        float(config.getoption("doctest_plus_rtol")),
    )
    OutputChecker.atol = max(
        float(config.getini("doctest_plus_atol")),
        float(config.getoption("doctest_plus_atol")),
    )

    use_rst = config.getini("doctest_rst") or config.option.doctest_rst
    file_ext = (
        config.option.text_file_format or config.getini("text_file_format") or "rst"
    )
    if use_rst:
    ...
    ext_comment_pairs = [
        pair.split("=") for pair in config.getini("text_file_comment_chars")
    ]
    ...
    class DocTestParserPlus(doctest.DocTestParser):
        ...
        def parse(self, s, name=None):
            ...
            if ext not in comment_characters:
                warnings.warn(
                    "file format '{}' is not recognized, assuming '{}' as the comment "
                    "character.".format(ext, comment_characters["rst"])
                )
                ext = ".rst"
            ...
            for entry in result:
                ...
                if isinstance(entry, six.string_types) and entry:
                    ...
                    if any(
                        [
                            re.match(
                                "{} doctest-skip-all".format(comment_char), x.strip()
                            )
                            for x in lines
                        ]
                    ):
                    ...
                    if match:
                        ...
                        if marker is None or (
                            marker.strip() == "win32" and sys.platform == "win32"
                        ):
                            skip_next = True
                            continue
                    ...
                    if config.getoption(
                        "remote_data", "none"
                    ) != "any" and entry.options.get(REMOTE_DATA):
                        entry.options[doctest.SKIP] = True
            ...
            if ignore_warnings_context_needed:
                result.insert(
                    0, doctest.Example(source=IGNORE_WARNINGS_CONTEXT, want="")
                )

            return result

    config.pluginmanager.register(
        DoctestPlus(DocTestModulePlus, DocTestTextfilePlus, config.option.doctestglob,),
        "doctestplus",
    )
