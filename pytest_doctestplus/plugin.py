...
def pytest_addoption(parser):
    parser.addoption('--doctest-plus',
                     action='store_true',
                     help="enable running doctests with additional features not found "
                          "in the normal doctest plugin")

    parser.addoption('--doctest-rst',
                     action='store_true',
                     help="Enable running doctests in .rst documentation. "
                          "This is no longer recommended, use --doctest-glob instead.")

    parser.addoption('--text-file-format',
                     action='store',
    ...
    parser.addoption('--doctest-plus-atol',
                     action='store',
                     help="set the absolute tolerance for float comparison",
                     default=1e-08)

    # Defaults to `rtol` parameter from `numpy.allclose`.
    parser.addoption('--doctest-plus-rtol',
                     action='store',
                     help="set the relative tolerance for float comparison",
                     default=1e-05)

    parser.addoption('--doctest-only',
                     action='store_true',
                     help="Test only doctests. Implies usage of doctest-plus.")

    parser.addini('text_file_format',
                  help="Default format for docs. This is no longer recommended, "
                       "use --doctest-glob instead.")

    parser.addini('doctest_optionflags',
                  help="option flags for doctests",
                  type='args',
                  default=['ELLIPSIS', 'NORMALIZE_WHITESPACE'])

    parser.addini('doctest_plus',
    ...
    parser.addini('doctest_norecursedirs',
                  help="like the norecursedirs option but applies only to doctest collection",
                  type='args',
                  default=())

    parser.addini('doctest_rst',
                  help="Run the doctests in the rst documentation",
                  default=False)
...
def pytest_configure(config):
    ...
    run_regular_doctest = (config.option.doctestmodules
                           and not config.option.doctest_plus)
    use_doctest_plus = (config.getini('doctest_plus')
                        or config.option.doctest_plus
                        or config.option.doctest_only)
    ...
    file_ext = (config.option.text_file_format
                or config.getini('text_file_format')
                or 'rst')
    if use_rst:
        config.option.doctestglob.append('*.{}'.format(file_ext))

    # override default comment characters
    ext_comment_pairs = [pair.split('=')
                         for pair in config.getini('text_file_comment_chars')]
    ...
    class DocTestModulePlus(doctest_plugin.DoctestModule):
        ...
        def collect(self):
            ...
            for test in finder.find(module):
                if test.examples and config.getoption('remote_data', 'none') != 'any':
                    self._patch_test_examples(test)
                    yield doctest_plugin.DoctestItem(test.name, self, runner, test)

        def _patch_test_examples(self, test):
            ...
            if ignore_warnings_context_needed:
                ex = doctest.Example(source=IGNORE_WARNINGS_CONTEXT, want='')
                test.examples.insert(0, ex)
    ...
    class DocTestParserPlus(doctest.DocTestParser):
        ...
        def parse(self, s, name=None):
            ...
            if ext not in comment_characters:
                warnings.warn("file format '{}' is not recognized, assuming '{}' as the "
                              "comment character.".format(ext, comment_characters['rst']))
                ext = '.rst'
            ...
            for entry in result:
                ...
                if isinstance(entry, six.string_types) and entry:
                    ...
                    if any(re.match('{} doctest-skip-all'.format(comment_char), x.strip())
                           for x in lines):
                        skip_all = True
                        continue
                    ...
                    last_lines = lines[-2:]
                    matcher = re.compile(r'{}\s+doctest-skip\s*::(\s+.*)?'.format(comment_char))
                    matches = [matcher.match(last_line) for last_line in last_lines]
                    ...
                    if match:
                        ...
                        if marker is None or marker.strip() == 'win32' and sys.platform == 'win32':
                            skip_next = True
                            continue

                    matcher = re.compile(r'{}\s+doctest-requires\s*::\s+(.*)'.format(comment_char))
                    matches = [matcher.match(last_line) for last_line in last_lines]
                    ...
                    not_any_remote = config.getoption('remote_data', 'none') != 'any'
                    if not_any_remote and entry.options.get(REMOTE_DATA):
                        entry.options[doctest.SKIP] = True
    ...
    config.pluginmanager.register(DoctestPlus(DocTestModulePlus, DocTestTextfilePlus,
        ...
        if hasattr(obj, '__doctest_skip__') or hasattr(obj, '__doctest_requires__'):
            if name is None and hasattr(obj, '__name__'):
                ...
                raise ValueError("DocTestFinder.find: name must be given "
                                 "when obj.__name__ doesn't exist: {!r}".format(type(obj)))
