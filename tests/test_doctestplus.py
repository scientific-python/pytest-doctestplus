...
def test_file_encoding(testdir, capsys, encoding):
    ...
    testdir.inline_run(p, "--doctest-plus-generate-diff", "--text-file-encoding", encoding)
    diff = dedent("""
         >>> print(2)
    -    4
    +    2
    ...
    testdir.inline_run(p, "--doctest-plus-generate-diff=overwrite", "--text-file-encoding", encoding)
    captured = capsys.readouterr()
    assert "Applied fix to the following files" in captured.out
...
def test_file_encoding_utf8(testdir, capsys, encoding):
    ...
    testdir.inline_run(p, "--doctest-plus-generate-diff=diff", "--text-file-encoding", encoding)
    diff = dedent("""
         >>> print(☆)
    -    ★
    +    ☆
