import doctest
from pytest_doctestplus.output_checker import OutputChecker, FLOAT_CMP, remove_trailing_whitespace
...
def test_remove_trailing_whitespace():
    assert remove_trailing_whitespace('foo  ') == 'foo'
    assert remove_trailing_whitespace('foo bar ') == 'foo bar'
    assert remove_trailing_whitespace('foo   bar ') == 'foo   bar'
    assert remove_trailing_whitespace('foo  baz  \nbar ') == 'foo  baz\nbar'
    assert remove_trailing_whitespace('foo \nbar \n') == 'foo\nbar'
    assert remove_trailing_whitespace('foo \nbar \n\n') == 'foo\nbar\n'
    assert (
        remove_trailing_whitespace("foo  bar\n---  ---\naa   4  ") ==
        remove_trailing_whitespace("foo  bar\n---  ---\naa   4")
    )
    assert (
        remove_trailing_whitespace("foo  bar\n---  ---\naa   4  ") !=
...
def test_ignore_trailing_whitespace(testdir, docstring_file):
    testdir.makeini("""
        [pytest]
        doctest_optionflags = IGNORE_TRAILING_WHITESPACE
        doctestplus = enabled
    """)
...
def test_ignore_trailing_whitespace_local(testdir, docstring_file):
    testdir.makeini("""
        [pytest]
        doctestplus = enabled
    """)
    ...
    p = docstring_file(r"""
        >>> print("foo  bar  ")  # doctest: +IGNORE_TRAILING_WHITESPACE
        foo  bar
    """)
    reprec = testdir.inline_run(p, "--doctest-plus")
    reprec.assertoutcome(passed=1)
...
def test_float_cmp_global(testdir, docstring_file):
    testdir.makeini("""
        [pytest]
